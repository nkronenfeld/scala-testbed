...
object Card {
  private def ranks = Array("2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A")

  def parse (text: String): Card = Card(ranks.indexOf(text.take(1)) + 2, text.drop(1))
}

object HandType extends Enumeration {
  type HandType = Value
  val Card, Pair, TwoPair, Triple, Straight, Flush, FullHouse, Quadruple, StraightFlush = Value
}

case class HandRank(rank: HandType.Value, further: Seq[Int]) extends Comparable[HandRank] {
  override def compareTo(that: HandRank): Int = {
    if (this.rank.id < that.rank.id) -1
    else if (this.rank.id > that.rank.id) 1
    else {
      (this.further zip that.further).find { case (a, b) => a != b }
        .map { case (a, b) => if (a < b) -1 else 1 }
        .getOrElse(0)
    }
  }
}
...
case class Hand (cards: Seq[Card]) {
  private val byValue = cards.map(_.value).distinct.map(v => v -> cards.count(_.value == v)).toMap

  val rank: HandRank = {
    val isFlush = cards.map(_.suit).distinct.length == 1
    val isStraight = cards.map(_.value).distinct.length == 5 && byValue.keySet.max == byValue.keySet.min + 4

    if (isFlush && isStraight) {
      HandRank(HandType.StraightFlush, Seq(byValue.keySet.max))
    } else if (byValue.values.max == 4) {
      HandRank(HandType.Quadruple, byValue.toSeq.sortBy(_._2).map(_._1))
    } else if (byValue.values.toSet == Set(3, 2)) {
      HandRank(HandType.FullHouse, byValue.toSeq.sortBy(_._2).map(_._1))
    } else if (isFlush) {
      HandRank(HandType.Flush, cards.map(_.value).sortBy(-_))
    } else if (isStraight) {
      HandRank(HandType.Straight, Seq(cards.map(_.value).max))
    } else if (byValue.values.toSet == Set(3, 1)) {
      HandRank(HandType.Triple, byValue.toSeq.filter(_._2 == 3).map(_._1) ++ byValue.toSeq.filter(_._2 == 1).map(_._1).sortBy(-_))
    } else if (byValue.count(_._2 == 2) == 2) {
      HandRank(HandType.TwoPair, byValue.toSeq.filter(_._2 == 2).map(_._1).sortBy(-_) ++ byValue.toSeq.filter(_._2 == 1).map(_._1))
    } else if (byValue.count(_._2 == 2) == 1) {
      HandRank(HandType.Pair, byValue.toSeq.filter(_._2 == 2).map(_._1) ++ byValue.toSeq.filter(_._2 == 1).map(_._1).sortBy(-_))
    } else {
      HandRank(HandType.Card, byValue.keySet.toSeq.sortBy(-_))
    }
  }

  override def toString: String = cards.mkString(s"${rank.rank}: [", ",", "]")
}

object Problem54 {
  def main (args: Array[String]): Unit = {
    val leftWinners = hands.split("\n").map(parseHand).map { case (left, right) =>
      (Hand(left), Hand(right))
    }.map { case (left, right) =>
      val c = left.rank compareTo right.rank
      val winner = if (c < 0) "right" else if (c > 0) "left" else "no-one"
      println(s"In ${left} vs ${right}, the winner is ${winner} [${c}]")
      if (c > 0) 1 else 0
    }.sum
    println(s"Left won ${leftWinners} times")
  }
  def parseHand (line: String): (Seq[Card], Seq[Card]) = {
    val cards = line.split(" ")
    (cards.take(5).map(Card.parse), cards.drop(5).map(Card.parse))
  }


  private val hands: String =
